...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F accel_frame = attitude.Rotate_BtoI(accel);
  
  predictedState(3) = curState(3) + dt * accel_frame.x;
  predictedState(4) = curState(4) + dt * accel_frame.y;
  predictedState(5) = curState(5) + dt * accel_frame.z - dt * CONST_GRAVITY;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  zFromX(0) = ekfState(6);
  float yaw_err = magYaw - ekfState(6);
